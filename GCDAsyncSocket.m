...
- (id)initWithDelegate:(id)aDelegate delegateQueue:(dispatch_queue_t)dq socketQueue:(dispatch_queue_t)sq
{
	if((self = [super init]))
	{
        ...
        //Unix DomainSocket:socket 框架上生成的一种IPC方式，用于IPC更有效率：1 不需要经过网络协议栈 ，2 不需要打包，计算校验和，维护序号，应答等只是将数据从一个进程转移到另一个进程。
		...
		{
            //--------创建穿行队列 DISPATCH_QUEUE_SERIAL ，用于网络数据读写的socket线程，用于处理网络数据的线程是否默认为主线程？
            //--------一个请求单独创建了一个线程执行一个串行队列来完成这个socket的一系列操作，并不意味着所有的socket都在一个线程上完成，比如异步执行的多个请求操作，在socket层次并不是为这些操作创建一个socket对象，把这些请求全部放在socket对象的socket队列中串行执行。而是创建了多个socket对象，每个对象分配一个线程来完成相应的socket操作，而这些socket读写操作的结果在代理回调的队列中执行（代理回调queue 是否默认为主线程，因为只有主线程可以操作界面，而在OC代码里面并没有直接设置代理回调queue，但是可以操作界面元素。）
            //因为socket的所有操作都是在sq上进行的，所以尽管存在线程间通信，并且没有添加锁，仍然可以保证线程安全。
        ...
        //--------这里使用了2次指针 ，方便之后的调用（之后仅仅写变量名称就行，不需要在写之前的&）
        ...
        //-------所要操作的整个数据，大多数都需要先放在prebuffer中进行保存，之后在进行操作，
...
- (BOOL)preConnectWithInterface:(NSString *)interface error:(NSError **)errPtr
{
	...
	//3 同一个socket对象中需要为串行执行，需要保障上一个执行完成，从新disconnect之后才能实现再次连接。
	if (![self isDisconnected]) // Must be disconnected
	{
		if (errPtr)
		{
	...
	if (interface)
	{
        ...
        //interface相当于网卡名称？函数获取了在特定网卡名称下的ipv4地址和ipv6地址
                ...
                error:(NSError **)errPtr
{
    ...
    //------copy地址，防止host在中途被变更。相当于重新allocnsstring 空间
	...
	dispatch_block_t block = ^{ @autoreleasepool {
		...
		//同样的操作，防止在socket执行过程中host变化引起的错误
		...
		#pragma clang diagnostic warning "-Wimplicit-retain-self"
            ...
            //-----server的地址数组。包括ipv4 ipv6地址 sockaddr_in6 sockaddr_in， 从dns中解析出来相关的地址。
	...
	if (dispatch_get_specific(IsOnSocketQueueOrTargetQueueKey))
        ...
        //这里同步执行了地址获取的操作 创建server地址这些费时的逻辑操作放在了异步线程中并发进行z
		dispatch_sync(socketQueue, block);
...
- (void)lookup:(int)aStateIndex didFail:(NSError *)error
{
	...
	//-----当前状态索引，当状态索引与当前索引不匹配时，客户端已经终止了连接，导致索引变更。
	if (aStateIndex != stateIndex)
	{
...
- (void)didConnect:(int)aStateIndex
{
    ...
    //setupStreamsPart1和setupStreamsPart2分别执行了那些操作。
                        ...
                        port:(uint16_t)port
{
	...
	{
		// ANY address
		//当没有interface时候 ，需要直接对sockaddr4 ，sockaddr6进行设置。
		struct sockaddr_in sockaddr4;
		...
		if ((getifaddrs(&addrs) == 0))
		{
			...
			while (cursor != NULL)
			{
				if ((addr4 == nil) && (cursor->ifa_addr->sa_family == AF_INET))
				{
					...
					{
                        //------否则将获取到的本地ip地址进行转化并比较。
						char ip[INET_ADDRSTRLEN];
						//inet_ntope二进制地址转化成点分十进制地址
						const char *conversion = inet_ntop(AF_INET, &nativeAddr4.sin_addr, ip, sizeof(ip));
						
						if ((conversion != NULL) && (strcmp(ip, iface) == 0))
						{
							// IP match
							//搜索到相应的ip则进行本地nativeAddr4设置。
							nativeAddr4.sin_port = htons(port);
...
- (BOOL)registerForStreamCallbacksIncludingReadWrite:(BOOL)includeReadWrite
{
	...
	//一般数据的传输并没有使用kCFStreamEventHasBytesAvailable选项，而只监听了kCFStreamEventErrorOccurred kCFStreamEventEndEncountered选项，tls触发的socket时候kCFStreamEventHasBytesAvailable
...
- (BOOL)addStreamsToRunLoop
{
	...
	if (!(flags & kAddedStreamsToRunLoop))
	{
		...
		//创建了一个cfstreamthread线程
		[[self class] startCFStreamThreadIfNeeded];
        //在创建的cfstreamthread线程上来操作当前对象的读写流；
...
- (BOOL)enableBackgroundingOnSocketWithCaveat:(BOOL)caveat
{
	...
	r1 = CFReadStreamSetProperty(readStream, kCFStreamNetworkServiceType, NSStreamNetworkServiceTypeVoIP);
//	r2 = CFWriteStreamSetProperty(writeStream, kCFStreamNetworkServiceType, kCFStreamNetworkServiceTypeVoIP);

    r2 = CFWriteStreamSetProperty(writeStream, kCFStreamNetworkServiceType, NSStreamNetworkServiceTypeVoIP);
	if (!r1 || !r2)
	{
		return NO;
	}
